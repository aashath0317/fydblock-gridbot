...
class GridManager:
    ...
    def update_zones_based_on_price(self, current_price: float) -> None:
        ...
        for price in self.price_grids:
            ...
            # FIX: Add small buffer (0.1%) to prevent "Ghost Orders" at the exact current price line due to noise.
            # We only consider it a BUY if price is clearly below market.
            is_buy_zone = price < (current_price * 0.999)

            ideal_state = GridCycleState.READY_TO_BUY if is_buy_zone else GridCycleState.READY_TO_SELL
    ...
    def get_order_size_for_grid_level(
        ...
        if total_grids == 0:
            return 0.0
        # FIX: Fixed Coin Quantity Logic (Strict)
        # We must use the FIXED Investment Amount from config, NOT the dynamic total_balance.
        investment = self.config_manager.get_investment_amount()
        if investment <= 0:
            # Fallback if config is missing investment (should not happen in prod)
            investment = total_balance

        # Formula: Total Investment / Number of Grids / Entry Price (Central Price)
        order_size = investment / total_grids / self.central_price
        return order_size
